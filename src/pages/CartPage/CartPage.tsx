...
export const CartPage = () => {
    ...
    const itemsAmount = (phone: Phone) => {
        return cart.filter(phoneToFilter => phone.itemId === phoneToFilter.itemId);
    }

    const removeAll = (phone: Phone) => {
        return cart.filter(phoneToRemove => phone.itemId !== phoneToRemove.itemId)
    }
            ...
            {cart.length > 0 && <div className='cart-content'>
                <ul className="cart__list">
                    {visibleItems.map(cartItem => {
                        return (
                            <div
                                key={cartItem.phoneId + Math.random()}
                                ...
                                <div className="cart-element__image-container">
                                    <img
                                        ...
                                        src={`https://raw.githubusercontent.com/mate-academy/product_catalog/main/public/${cartItem.image}`}
                                        alt={cartItem.name}
                                    />
                                </div>
                                <h2 className="cart-element__title">{cartItem.name}</h2>
                                <div className="cart-element__count count">
                                    <button className="count__btn"
                                        disabled={itemsAmount(cartItem).length === 1}
                                        onClick={() => removeFromCart(cartItem)}
                                    >-</button>
                                    <span className="count__value">{itemsAmount(cartItem).length}</span>
                                    <button
                                        className="count__btn"
                                        onClick={() => addToCart(cartItem)}
                                    >+</button>
                                </div>
                                <div className="cart-element__value">${cartItem.fullPrice}</div>
                            </div>
                        )
                    })}
